...
{
    public partial class SprawdzKnnSumNajmniejszych : Form
    {
        public SprawdzKnnSumNajmniejszych()
        {
            InitializeComponent();
        }

        private void SprawdzKnnSumNajmniejszych_Load(object sender, EventArgs e)
        {
            ...
            /* try
             {
                 ...
                 if (czyIstnieje.czyIstnieje)
                 {
                     ...
                     KHH kHH = new KHH();
                     int kCzydobre = kHH.JakieKsprawdzanieSumNajmniejszych(normalizujprzed.wartosciPoNormalizacji, wczytaj.dlugosc, wczytaj.numerKolumnyWartosciDecyzyjnej);
        ...
        string sprawdzSwegoKnn(int szerokosc, WczytajPlik wczytaj, int k, int metryka, string[,] ZbiorWartosciWzorcowych)//baza próbek wcześniej znormalizowana
        {
            ...
            for (int i = 0; i < wczytaj.dlugosc + 1; i++)
            {
                ...
                string decyzja = hH.KlasyfikatorMetodaDruga(k, metryka, ZbiorWartosciWzorcowych, obiekt);
